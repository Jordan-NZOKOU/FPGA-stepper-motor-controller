...
def update_label(label, stringvar):
    ...
    t = value.get()
    ser.write(packUnsignedCharAsUChar(t))
    t2 = value2.get()
    ser.write(packUnsignedCharAsUChar(t2))
...
root = Tk()
root.title("Pilotage moteur pas à pas TP instrumentation 2")
...
spin = Spinbox(root,values=('COM1','COM2','COM3','COM4','COM5','COM6','COM7','COM8','COM9','COM10','COM11'),fg="black",textvariable=values)
...
value =IntVar()
value.set(0)
scale1 = Scale(frame, from_=0, to=255,resolution=10,fg="blue", showvalue=True,width = 15,font=('arial', 10), label='valeur fréquence',
...
value2 =IntVar()
value2.set(0)
scale2 = Scale(frame, from_=0, to=2,resolution=1,fg="blue", showvalue=True,width = 15,font=('arial', 10), label='sens de rotation',
variable=value2, tickinterval=1, orient='h')
entry = Entry(frame, textvariable=value2)
scale2.grid(row=3, column=0)
